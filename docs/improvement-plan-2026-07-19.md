...
> **Superseded on 2026-07-20 by [`improvement-plan-2026-07-20.md`](./improvement-plan-2026-07-20.md).** Milestones 0–1 completed under this plan; Milestone 2's remaining scope and Milestones 3–6 carry forward into the new plan's phases. This document is retained as the execution record.

**Status date:** 2026-07-20
**Execution status:** Milestone 2 command set complete; all seven transactional commands are implemented and tested, and the superseding plan now includes the first append-only correction command for cellar transfers
...
|---|---|---|
| Mechanical health | dependency audit, `npm run lint`, `npm test`, `npm run build`, the fresh bundle-budget assertion, and the live production boot smoke pass; 744 default-suite assertions pass across 106 test files, with 19 additional PostgreSQL integration assertions in the required CI gate | Preserve the green baseline, but add missing multi-client and browser coverage |
...
| Testing | Vitest coverage is broad and the required release job now applies migrations against PostgreSQL 16, rejects schema drift, and runs tenant-isolation integration tests; no browser E2E runner is configured | Continue with offline/concurrency integration tests and representative browser journeys |
...
### Current execution note — 2026-07-20

- The required `Release gates` job now provisions an isolated PostgreSQL 16 service, applies the exact committed migrations through the production migration wrapper, and fails on schema drift before running the ordinary verification pipeline.
- A dedicated PostgreSQL suite verifies that every committed migration finished successfully, duplicate lot/vessel display IDs remain isolated inside authoritative organization JSONB state, compare-and-swap updates cannot overwrite a newer version, and organization cascades do not affect another tenant.
- Ordinary local/unit tests continue without a database; `npm run test:postgres` is intentionally isolated and requires `TEST_DATABASE_URL` or `DATABASE_URL` pointing to a disposable database.
- The shared command executor now claims `(organizationId, commandId)` with conflict-safe PostgreSQL semantics, hashes the command type and canonical JSON payload, and commits the domain callback plus replayable result in the same transaction. A callback failure rolls back the domain write and claim together; reusing an id with a different request returns the stable `idempotency_key_reused` conflict.
- An authenticated, organization-scoped `GET /api/commands/:commandId` endpoint lets reconnecting clients recover a completed result without leaking another tenant's command. It returns stable validation, not-found, and retryable storage-unavailable codes.
- Fast contract/route tests cover canonical hashing, exact replay, rollback/retry, stable errors, stale organization context, and tenant scoping. The PostgreSQL suite additionally races duplicate requests and proves that only one state mutation commits.
- `POST /api/commands/cellar.transfer` now executes the first business command through this contract. It holds a PostgreSQL row lock for the winery state, updates source/destination vessels, lot balances and genealogy, the transfer ledger, state version, and replay result atomically, and validates capacity, cleanliness, lot consistency, record identity, and complete workflow permission at the server boundary.
- Duplicate transfer requests replay one result without a second ledger effect. Distinct commands for the same winery serialize on the organization-state row, preventing two sessions from spending the same source volume. The PostgreSQL suite covers both races.
- Online clients persist the exact transfer command ID and payload before sending, block organization switching while the result is uncertain, and recover the same result after a timeout or reload. A verified acknowledgement hydrates authoritative vessels, lots, and transfers without issuing a duplicate sync write. Offline and development JSON-backend work continues through the existing durable local sync queue.
- Command-shaped transfers now use `cellar.transfer.reverse`: a second idempotent command restores captured vessel/lot state only when no dependent work intervened, retains the original ledger entry, appends correction evidence, and removes the legacy partial client rollback.
- `POST /api/commands/cellar.bottling` now commits lot volume/history, the bottling run, packaging deductions, packaging/service costs, and optional finished-goods placement in one idempotent organization-state transaction. The server derives currency from company state and validates format counts, exact volume, record collisions, packaging availability, location identity, and warehouse capacity before mutation.
- Bottling permissions remain feature-sensitive: Winemakers can execute the core lot/run command, while packaging/cost and finished-goods effects require their existing inventory/cost and storage permissions. A forbidden optional ledger cannot be smuggled through the compound payload.
- The lazy bottling client saves the exact run intent before sending, restores and freezes it after reload, replays the same command after an uncertain response, hydrates authoritative collections after acknowledgement, and retains the established offline/development whole-state fallback without increasing the initial application bundle.
- Command-created bottling runs are protected from the legacy partial rollback path until a server-owned reversal command can compensate every linked ledger together.
- Domain and HTTP tests cover complete posting, permissions, rollback-on-failure, volume/stock/capacity conflicts, deterministic identifiers, replay, and durable client recovery. PostgreSQL races prove that duplicate requests produce one run and that distinct commands cannot double-spend packaging stock after both read the same initial state.
- `POST /api/commands/sales.stock` now owns reservation, direct dispatch, reservation fulfillment, and cancellation through one discriminated command contract. It derives currency and cost-per-bottle from authoritative organization state and uses one active-reservation calculation for every path.
- Reservation and direct dispatch reject quantities above unreserved on-hand stock. Fulfillment excludes its own reservation while preserving every competing reservation, then links the fulfilled order, sales dispatch, and outbound stock movement in one transaction. Cancellation releases only an order that is still reserved.
- The Sales client persists one exact action before sending, restores captured reservation/dispatch inputs after reload, freezes every other Sales control while acknowledgement is uncertain, and exposes an explicit same-command replay. Verified responses hydrate authoritative orders, dispatches, and movements; offline/development use the same pure command through the established sync queue.
- Command-created dispatches cannot enter the legacy tombstone-based partial delete path until a complete server-owned reversal command is added.
- Domain, client, route, permission, sync, and PostgreSQL coverage prove server-derived financials, exact replay, rollback-on-rejection, complete fulfillment linkage, cancellation, and row-lock serialization when two sessions race to reserve the same bottles.
- `POST /api/commands/cellar.harvest-intake` now commits the structured intake, created wine lot, signed audit-chain event, optional vineyard-harvest link, optional empty-vessel fill, and optional fruit-cost posting as one idempotent organization-state transaction. Company currency, region, vineyard identity, and linked references are resolved from authoritative state.
- The intake invariant layer rejects reused or mismatched harvests, stale/generated record IDs, invalid weights and chemistry, non-empty or unclean destination vessels, and capacity conflicts before any effect commits. Distinct commands serialize on the organization row so one harvest cannot create two intakes; duplicate requests replay one result.
- The Grape Intake client persists the exact command and generated intake/lot/audit IDs before sending, restores captured form values after reload, freezes the form while acknowledgement is uncertain, and can replay the same command. Verified responses hydrate all six affected collections; offline/development fallback executes the identical pure command.
- Vazi no longer performs the fabricated partial lot/harvest mutation in the live application. Its harvest handoff opens the canonical intake review with the harvest prefilled, and the harvest is marked received only when the full intake command commits.
- Domain, client, route, permission, and PostgreSQL tests cover complete posting, authoritative financial/origin data, exact replay, rollback, optional-feature authorization, harvest/vessel conflicts, and concurrent duplicate/distinct requests.
- `POST /api/commands/cellar.fermentation-complete` now promotes one existing final fermentation reading to completion evidence, transitions the lot from fermentation to stabilization, records the vessel's lifecycle operation without changing cooling controls, and appends a signed audit-chain event in one idempotent organization-state transaction.
- The completion boundary rejects stale/non-fermenting lots, empty or mismatched vessel assignments, readings from another lot or vessel, invalid stored chemistry, duplicate completion evidence, and audit-ID collisions before any mutation. Completion requires fermentation-update, lot-update, vessel-update, and audit-write authority as one compound permission.
- The Fermentation client persists the exact lot, vessel, final-reading, audit, and command identifiers before sending; it restores and freezes an uncertain completion after reload, offers exact-command replay, hydrates authoritative collections after acknowledgement, and uses the identical pure transaction for offline/development fallback. The existing free-text additives field has no inventory identity or quantity, so completion deliberately creates no fabricated material or cost effect.
- Domain, client, route, permission, and PostgreSQL tests cover complete posting, signed evidence, cooling-control preservation, exact replay, rollback, authorization, mismatched state, and concurrent duplicate/distinct completion attempts.
- `POST /api/commands/cellar.operation` now commits the operation ledger, lot timeline and optional volume change, optional operating-vessel state, optional material deduction, derived material cost, and signed audit-chain event in one idempotent organization-state transaction. Currency and material unit cost come from authoritative organization state.
- The command rejects sold or missing lots, stale/generated record IDs, lot/vessel assignment or volume mismatch, invalid destination references, unsupported type-specific fields, capacity conflicts, invalid stored stock/cost data, derived-cost collisions, and insufficient material before mutation. Material overdraw is no longer silently clamped to zero.
- Optional permissions remain feature-sensitive: a Winemaker can record the core lot/audit operation and permitted vessel context, while material deduction plus its cost posting requires inventory-update and cost-create authority. Forbidden optional effects are rejected before an idempotency claim is created.
- The Operations client persists the exact form values and generated operation/audit IDs before sending, restores and freezes an uncertain form after reload, exposes same-command replay, hydrates all six authoritative collections after acknowledgement, and executes the same pure transaction for offline/development fallback.
- Domain, client, route, permission, and PostgreSQL coverage proves complete posting, authoritative costing, JSON-safe durable results, exact replay, rollback, optional-feature authorization, quantity/reference conflicts, and duplicate/distinct races against the same material stock.
- General storage movement is now command-owned: source-linked receipt, paired relocation, and explicit adjustment update finished-goods availability, capacity-constrained movement ledgers, and bottling placement evidence atomically without duplicating sales or bottling storage effects. The durable client restores an uncertain intent, and sync validation shares the same receive/relocation link semantics.
- The shared reversal reference/receipt contract and first implementation, `cellar.transfer.reverse`, are complete under the superseding plan. Domain, route, client, sync, permission, lineage, and PostgreSQL race coverage enforce append-only corrections and reject stale or forged compensation.
