...
**Status:** In progress — Milestone 0 trust repair  
...
- validation passes: TypeScript, 400 automated tests across 71 files, production build, and live browser QA for Owner, Lab Technician, and Cellar Worker navigation/action matrices in English and Georgian, including compound transfer and fermentation behavior with zero browser console errors.

UX-002 remains **in progress** for the remaining operational forms. The next bounded rollout is permission-correct mutation controls and authorized cross-collection transactions for Vazi, grape intake, cellar operations, storage, costs, and sales. These flows currently rely on server rejection for some role combinations and need component-level action contracts plus transaction-safe backend commands.
