                    ...
                    <table className="w-full text-left text-[11px]">
                      <tbody className="divide-y divide-stone-50 dark:divide-stone-800">
                        {lotEntries.map(([lotId, n]) => (
                          <tr key={lotId}><td className="py-1.5 text-stone-700 dark:text-amber-50">{lotName(lotId)}</td><td className="py-1.5 text-right font-mono font-bold text-[#4e0e15] dark:text-amber-300">{n.toLocaleString()}</td></tr>
                        ))}
                      </tbody>
                    </table>
                  )}
