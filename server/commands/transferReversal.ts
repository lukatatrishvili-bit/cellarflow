...
export interface ExecuteCellarTransferReversalInput {
...
/** Compensate one transfer while holding the same organization row used by new workflow commands. */
export async function executeCellarTransferReversalCommand(
  prisma: PrismaClient,
  input: ExecuteCellarTransferReversalInput,
    ...
    async (transaction) => {
      ...
      const updated = await transaction.organizationState.update({
        ...
        data: {
          ...
          version: { increment: 1 },
          updatedBy: `command:${TRANSFER_REVERSAL_COMMAND_TYPE}:${input.actorUsername}`,
        },
      });
      return {
        ...applied.result,
...
export function transferReversalCommandResult(
  outcome: IdempotentCommandOutcome<Prisma.JsonValue>,
): TransferReversalCommandResult {
  return outcome.result as unknown as TransferReversalCommandResult;
}
